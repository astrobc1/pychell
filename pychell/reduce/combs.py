...
def compute_lsf_width_all(times_sci, times_cal, wls_cal_scifiber, lfc_cal_scifiber, do_orders=None):
    ...
    for order_index in range(n_orders):
        ...
        for i in range(n_cal_spec):
            try:
                lsfwidths_cal_scifiber[order_index, i] = compute_lsf(wls_cal_scifiber[:, order_index, i], lfc_cal_scifiber[:, order_index, i])
            except:
...
def compute_lsf_width(lfc_wave, lfc_flux):
    ...
    lfc_centers_wave_theoretical = gen_theoretical_peaks(lfc_wave)

    # Remove background flux
    background = estimate_background(lfc_wave, lfc_flux)
    lfc_flux_no_bg = lfc_flux - background
    lfc_peak_max = pcmath.weighted_median(lfc_flux_no_bg, percentile=0.75)
    
    # Estimate continuum
    continuum = estimate_continuum(lfc_wave, lfc_flux_no_bg)
...
def compute_wls_all(times_sci, times_cal, wave_estimate_sci, wave_estimate_cal, lfc_sci_calfiber, lfc_cal_calfiber, lfc_cal_scifiber, do_orders=None, method="interp"):
    ...
    for order_index in range(n_orders):
        ...
        for i in range(n_cal_spec):
            ...
            try:
                wls_cal_scifiber[:, order_index, i] = compute_wls(wave_estimate_sci[:, order_index], lfc_cal_scifiber[:, order_index, i])
            except:
                print(f"Warning! Could not compute wls for order {order_index+1} cal spectrum {i+1}")
            
            try:
                wls_cal_calfiber[:, order_index, i] = compute_wls(wave_estimate_cal[:, order_index], lfc_cal_calfiber[:, order_index, i])
            except:
                print(f"Warning! Could not compute wls for order {order_index+1} cal spectrum {i+1}")
        ...
        if times_sci is not None:
            for i in range(n_sci_spec):
                ...
                try:
                    wls_sci_calfiber[:, order_index, i] = compute_wls(wave_estimate_cal[:, order_index], lfc_sci_calfiber[:, order_index, i])
                except:
...
def compute_wls(wave_estimate, lfc_flux, df, f0, poly_order=None, n_knots=None, peak_separation=10):
    ...
    lfc_centers_pix, lfc_centers_wave = compute_peaks(wave_estimate, lfc_flux, f0, df, peak_separation=10)
...
def compute_peaks(wave_estimate, lfc_flux, f0, df, peak_separation=10):

    # Generate theoretical LFC peaks
    lfc_centers_wave_theoretical = gen_theoretical_peaks(wave_estimate, f0, df)
    ...
    # Remove background flux
    background = estimate_background(wave_estimate, lfc_flux, peak_separation)
    lfc_flux_no_bg = lfc_flux - background
    lfc_peak_max = pcmath.weighted_median(lfc_flux_no_bg, percentile=0.75)
    
    # Estimate continuum
    continuum = estimate_continuum(wave_estimate, lfc_flux_no_bg)
    lfc_flux_norm = lfc_flux_no_bg / continuum

    # Estimate peaks in pixel space (just indices)
    peaks = scipy.signal.find_peaks(lfc_flux_norm, height=np.full(nx, 0.5), distance=0.8*peak_separation)[0]
    peaks = np.sort(peaks)
    ...
    for i in range(len(good_peaks)):
        ...
        bounds = [(0.8, 1.5), (p0[1] - peak_separation / 2, p0[1] + peak_separation / 2), (0.25 * p0[2], 4*p0[2])]
        opt_result = scipy.optimize.minimize(fit_peak, p0, args=(xarr[use], lfc_flux_no_bg[use] / np.nanmax(lfc_flux_no_bg[use])), method='Nelder-Mead', bounds=bounds)
        pbest = opt_result.x
...
def gen_theoretical_peaks(lfc_wave, f0, df, n_left=10_000, n_right=10_000):
...
def estimate_background(lfc_wave, lfc_flux, peak_separation=10):
    background = pcmath.generalized_median_filter1d(lfc_flux, width=int(2 * peak_separation), percentile=0.01)
    return background

def estimate_continuum(lfc_wave, lfc_flux, peak_separation=10):
    continuum = pcmath.generalized_median_filter1d(lfc_flux, width=int(2 * peak_separation), percentile=0.99)
    return continuum
