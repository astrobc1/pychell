...
class OptimalExtractor(SpectralExtractor):
    ...
    def _extract_trace(data, image, trace_map_image, trace_dict, badpix_mask, read_noise=None, remove_background=True, background_smooth_poly_order=3, background_smooth_width=51, flux_cutoff=0.05, trace_pos_poly_order=4, oversample=4, n_trace_refine_iterations=3, trace_pos_refine_window=None, n_extract_iterations=3, badpix_threshold=5, extract_orders=None, _extract_aperture=None):
        ...
        # TEMP
        #trace_profile_cspline = OptimalExtractor.compute_trace_profile(trace_image, badpix_mask, trace_positions, background, remove_background, oversample)
        #spec1d, spec1d_unc = OptimalExtractor.boxcar_extraction(trace_image, badpix_mask, trace_profile_cspline, trace_positions, extract_aperture=[5, 5], remove_background=False)
    ...
    def optimal_extraction(trace_image, badpix_mask, trace_profile_cspline, trace_positions, extract_aperture, remove_background=True, background=None, background_err=None, read_noise=0):
        ...
        bb = np.full(nx, fill_value=np.nan, dtype=float)
        ...
        for x in range(nx):
            ...
            # Copy arrs
            S_x = np.copy(trace_image[:, x])
            M_x = np.copy(badpix_mask[:, x])
            ...
            good = np.where((yarr >= trace_positions[x] - extract_aperture[0] - 0.5) & (yarr <= trace_positions[x] + extract_aperture[1] + 0.5) & (M_x == 1))[0]
            #good = np.where(M_x)[0]
            #if x == 1732:
            #    breakpoint()
            ...
            A = (np.nansum(w * P * S) - B * np.nansum(w * P)) / np.nansum(w * P**2)

            spec[x] = A * np.nansum(P)
            bb[x] = B

            # P_use = data_use / np.nansum(data_use)
            # w_use = (P_use / var_use) / np.nansum(P_use**2 / var_use)
            # w_use /= np.nansum(w_use)
            # spec[x] = np.nansum(w_use * data_use)

            #P_use = P_x[good]

            #breakpoint()
            #spec[x] = np.nansum(P_use**2 * data_use) * np.nansum(P_use) / np.nansum(P_use**2 * np.nansum(P_use**2))
        ...
        # Return
        breakpoint()
        matplotlib.use("MacOSX"); plt.plot(spec); plt.show()
        spec_unc = np.sqrt(spec)
    ...
    @staticmethod
    def compute_model2d(trace_image, badpix_mask, spec1d, trace_profile_cspline, trace_positions, extract_aperture, remove_background=True, background=None, background_err=None, smooth=True):
