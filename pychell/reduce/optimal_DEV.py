...
class OptimalExtractor(SpectralExtractor):
    ...
    def __init__(self, extract_orders=None, chunk_width=None, chunk_overlap=0.5, oversample=1,
                 trace_pos_chunk_poly_order=2, trace_pos_poly_order=4, trace_pos_refine_window=9,
    ...
    def _extract_trace(data, image, trace_map_image, trace_dict, badpix_mask,
        ...
        for i in range(n_chunks):
            ...
            trace_profile_cspline_chunks.append(trace_profile_cspline)

            
        #OptimalExtractor.compute_vertical_trace_profile_2d(trace_profile_cspline_chunks)
        matplotlib.use("MacOSX")
        xhr = np.arange(-13, 10, 0.001)
        tps = np.full((len(xhr), n_chunks), np.nan)
        for i in range(n_chunks):
            tp = trace_profile_cspline_chunks[i](xhr)
            tps[:, i] = tp
            plt.plot(xhr, tp)
        plt.show()
        breakpoint()
