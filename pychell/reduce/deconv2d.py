...
class LSQR2dExtractor(GaussianDeconv2dExtractor):
    ...
    def deconv2dextraction(trace_image, badpix_mask, trace_positions, sigma, q, theta, extract_aperture, background=None, remove_background=True, read_noise=0, chunk_width=200, distrust_width=20, oversample=1, damp=0.1):
        ...
        for i in range(n_chunks):
            ...
            syhr = slinalg.lsqr(Aprimeflat, Sprimeflat, damp=0.01, x0=x0)[0]
...
class Bayesian2dExtractor(GaussianDeconv2dExtractor):
    ...
    def _extract_trace(data, image, trace_map_image, trace_dict, badpix_mask, read_noise=None, remove_background=True, background_smooth_poly_order=3, background_smooth_width=51, flux_cutoff=0.05, trace_pos_poly_order=4, oversample=1, n_trace_refine_iterations=3, n_extract_iterations=3, trace_pos_refine_window=5, badpix_threshold=5, _extract_aperture=None, sigma=None, q=None, theta=None, chunk_width=200, distrust_width=20):
        ...
        # Extract Aperture
        if _extract_aperture is None:
            extract_aperture = Bayesian2dExtractor.compute_extract_aperture(trace_profile_cspline)
        else:
            extract_aperture = _extract_aperture
        ...
        for i in range(n_extract_iterations):
            ...
            # Deconv extraction
            spec1d, spec1d_unc = Bayesian2dExtractor.deconv2dextraction(trace_image, badpix_mask, trace_positions, sigma, q, theta, extract_aperture, background, remove_background, read_noise, chunk_width, distrust_width, oversample)

            # Re-map pixels and flag in the 2d image.
            if i < n_extract_iterations - 1:

                # Create the 2d model
                model2d = Bayesian2dExtractor.compute_model2d(trace_image, badpix_mask, spec1d, trace_positions, sigma, q, theta, extract_aperture, background, remove_background)

                # Flag bad pixels
                Bayesian2dExtractor.flag_pixels2d(trace_image, badpix_mask, model2d, badpix_threshold)
    ...
    def deconv2dextraction(trace_image, badpix_mask, trace_positions, sigma, q, theta, extract_aperture, background=None, remove_background=True, read_noise=0, chunk_width=100, distrust_width=20, oversample=1):
        ...
        # Chunks
        chunks = Bayesian2dExtractor.generate_chunks(trace_image, badpix_mask_cp, chunk_width=chunk_width)
        n_chunks = len(chunks)
        ...
        for i in range(n_chunks):
            ...
            # Generate Aperture tensor for this chunk
            A = Bayesian2dExtractor.generate_A(S, xarr_aperture, tp_hr, sigma_hr, q_hr, theta_hr, extract_aperture)
            ...
            # Call sparse extraction
            syhr = Bayesian2dExtractor.map_deconv(Aprimeflat, Sflat, x0, n_iterations=1)
            syhr = Bayesian2dExtractor.map_deconv(Aprimeflat, Sflat, x0, n_iterations=20)

            # Bin back to detector grid
            sy = Bayesian2dExtractor.bin_spec1d(syhr, oversample)
    ...
    @staticmethod
    @njit(nogil=True)
    def map_deconv(Aflat, Sflat, x0, n_iterations=5):

        x = np.copy(x0)
        n_apertures = len(x0)
        n_data_pixels = Aflat.shape[0]

        for i in range(n_iterations):
            x_old = np.copy(x)
            for m in range(n_apertures):
                s = 0
                for j in range(n_data_pixels):
                    if np.all(Aflat[j, :] == 0):
                        continue
                    ss = 0
                    for p in range(n_apertures):
                        ss += Aflat[j, p] * x_old[p]
                    if ss != 0:
                        s += Aflat[j, m] * (Sflat[j] / ss - 1)
                x[m] = x_old[m] * np.exp(s)
        return x