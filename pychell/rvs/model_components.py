...
class SpectralComponent:
    ...
    def __init__(self, blueprint, order_num=None, wave_logspace=False, flux_logspace=False):
        ...
        # Wave and flux space
        self.wave_logpsace = wave_logspace
        self.flux_logspace = flux_logspace
    ...
    # Called after each iteration, must implement
    def update(self, forward_model, iter_num, *args, **kwargs):
        pass
...
class MultModelComponent(SpectralComponent):
    ...
    Attributes:
        ...
        flux_logspace (bool): Whether or not this model is generated in logspace. It is recommended the log is accounted for in the corresponding build_full method, not here (for now).
    ...
    def __init__(self, blueprint, wave_bounds, order_num=None, wave_logspace=False, flux_logspace=False):
        
        # Call super method
        super().__init__(blueprint, order_num=order_num, wave_logspace=wave_logspace, flux_logspace=flux_logspace)
        
        # The wavelength bounds (accounting for cropped pixels)
        self.wave_bounds = wave_bounds
        
        # Whether or not the model is computed in logspace
        self.flux_logspace = flux_logspace
...
class EmpiricalMult(MultModelComponent):
    ...
    def __init__(self, blueprint, wave_bounds, order_num=None, wave_logspace=False, flux_logspace=False):
        ...
        # Call super method
        super().__init__(blueprint, wave_bounds, order_num=order_num, wave_logspace=wave_logspace, flux_logspace=flux_logspace)
...
class StarModel(TemplateMult):
    ...
    def update(self, forward_model, iter_num):
        if iter_num == 0 and not self.enabled:
            self.enabled = True
            self.initial_parameters[self.par_names[0]].setv(value=-1*forward_model.data.bc_vel, vary=True)
...
class StarModelOrderDependent(TemplateMult):
    ...
    def load_template(self, nx, *args, pad=15, **kwargs):
        if self.from_synthetic:
            ...
            good = np.where((wave_init > self.wave_bounds[0] - pad) & (wave_init < self.wave_bounds[1] + pad))[0]
            ...
            wave_star = np.linspace(self.wave_bounds[0] - pad, self.wave_bounds[1] + pad, num=nx)
...
class LSFModel(SpectralComponent):
    ...
    def __init__(self, blueprint, dl, nx_model, order_num=None, wave_logspace=False, flux_logspace=False):
        ...
        if self.nx % 2 == 0:
            ...
            self.x = np.arange(-(int(self.nx / 2)), int(self.nx / 2) + 1, 1) * self.dl
    ...
    def convolve_flux(self, raw_flux, pars=None, lsf=None):
        ...
        if self.nx % 2 == 0:
            ...
            padded_flux = np.pad(raw_flux, pad_width=(int(self.nx / 2), int(self.nx/2)), mode='constant', constant_values=(raw_flux[0], raw_flux[-1]))
...
class LSFHermiteModel(LSFModel):
    ...
    def __init__(self, blueprint, dl, nx_model, order_num=None, wave_logspace=False, flux_logspace=False):
        ...
        # Call super
        super().__init__(blueprint, dl, nx_model, order_num=order_num, wave_logspace=wave_logspace, flux_logspace=flux_logspace)
    ...
    def update(self, forward_model, iter_num):
        if iter_num == self.n_delay - 1 and not self.enabled:
            ...
            for par in self.par_names:
                self.initial_parameters[par].vary = True
...
class WavelengthSolutionModel(SpectralComponent):
    ...
    def __init__(self, blueprint, wave_bounds, pix_bounds, nx, order_num=None, wave_logspace=False, flux_logspace=False):
    ...
    def estimate_endpoints(data, blueprint, pix_bounds):
        ...
        if hasattr(data, 'wave_grid'):
            ...
            wavesol_base_wave_set_points = np.array([blueprint['base_set_point_1'][data.order_num - 1], blueprint['base_set_point_2'][data.order_num - 1], blueprint['base_set_point_3'][data.order_num - 1]])
...
class WaveSolModelFull(WavelengthSolutionModel):
    ...
    def __init__(self, blueprint, wave_bounds, pix_bounds, nx, order_num=None, wave_logspace=False, flux_logspace=False):
        ...
        # Call super method
        super().__init__(blueprint, wave_bounds, pix_bounds, nx, order_num=order_num, wave_logspace=wave_logspace, flux_logspace=flux_logspace)
        ...
        self.base_wave_zero_points = np.array([blueprint['base_set_point_1'][self.order_num - 1], blueprint['base_set_point_2'][self.order_num - 1], blueprint['base_set_point_3'][self.order_num - 1]])
...
class WaveSolModelLegendre(WavelengthSolutionModel):
    ...
    def init_parameters(self, templates_dict):
        self.initial_parameters = OptimParameters.Parameters()
        self.initial_parameters.add_parameter(OptimParameters.Parameter(name=self.par_names[0], value=self.base_wave_zero_points[1], minv=self.base_wave_zero_points[1] - 0.5, maxv=self.base_wave_zero_points[1] + 0.5, mcmcscale=0.001, vary=True))
        self.initial_parameters.add_parameter(OptimParameters.Parameter(name=self.par_names[1], value=self.blueprint['coeff1'][1], minv=self.blueprint['coeff1'][0], maxv=self.blueprint['coeff1'][2], mcmcscale=0.001))
        self.initial_parameters.add_parameter(OptimParameters.Parameter(name=self.par_names[2], value=self.blueprint['coeff2'][1], minv=self.blueprint['coeff2'][0], maxv=self.blueprint['coeff2'][2], mcmcscale=0.001))
